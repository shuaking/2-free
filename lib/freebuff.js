...
const MODEL_TO_AGENT = {
  ...
  'google/gemini-2.5-flash': 'file-picker-max',
  'google/gemini-2.5-pro': 'thinker-with-files-gemini',
  'google/gemini-3.1-flash-lite-preview': 'file-picker-max',
  'google/gemini-3.1-pro-preview': 'thinker-with-files-gemini',
  'anthropic/claude-3.7-sonnet': 'thinker-with-files-claude',
  'anthropic/claude-sonnet-4': 'thinker-with-files-claude',
  'anthropic/claude-opus-4': 'thinker-with-files-claude',
};
...
function getAgentId(model) {
  const normalized = resolveModel(model);
  return MODEL_TO_AGENT[normalized] || MODEL_TO_AGENT[getDefaultModel()];
}
...
async function createChatCompletionStream(openaiBody, onChunk) {
  const model = resolveModel(openaiBody?.model || getDefaultModel());
  const normalizedBody = { ...(openaiBody || {}), model };
  ...
  const { account, index, strategy: actualStrategy } = selectAccount(normalizedBody?.accounts, model, strategy, preferredIndex);
  const agentId = getAgentId(model);
  const token = account.authToken;

  let runId = await getOrCreateAgentRun(token, agentId);
  let freebuffBody = openaiToFreebuffBody(normalizedBody, runId);
  freebuffBody.stream = true;
  ...
  if (!response.ok) {
    if (response.status === 400 || response.status === 404) {
      ...
      freebuffBody = openaiToFreebuffBody(normalizedBody, runId);
      freebuffBody.stream = true;

      const retryResponse = await fetch(`${API_BASE}/api/v1/chat/completions`, {
        method: 'POST',
        headers: {
          Accept: 'text/event-stream',
          'Content-Type': 'application/json',
      ...
      if (!retryResponse.ok) {
        ...
        throw new Error(`流式请求失败: HTTP ${retryResponse.status} ${text}`);
      }

      return { response: retryResponse, runId, model, accountMeta };
    }

    const text = await response.text();
    throw new Error(`流式请求失败: HTTP ${response.status} ${text}`);
  }

  return { response, runId, model, accountMeta };
}
